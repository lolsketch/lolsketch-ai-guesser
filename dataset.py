...
def get_data_loaders(train_bs, val_bs):
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=train_bs, shuffle=False) # No need to shuffle since we are ignoring the index in __getitem__
...
def plot_images(images, labels, classes):
    ...
    for i in range(4):
        ...
        ax.set_xlabel(classes[labels[i]], fontsize=10)
        ax.set_xticks(range(0, images[i].shape[2], 10))
        ax.set_yticks(range(0, images[i].shape[1], 10))
